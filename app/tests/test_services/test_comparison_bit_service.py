...
async def test_comparison_bit_service(comparison_bit_service: ComparisonBitService):
    integer_one_as_byte = (1).to_bytes(128, byteorder="big")
    integer_zero_as_byte = (0).to_bytes(128, byteorder="big")
    ...
    current_comparison_bit = await comparison_bit_service.save_bit(
        bytes=integer_one_as_byte, timestamp=1 + 60 * 60 * 24, source="s2"
    )
